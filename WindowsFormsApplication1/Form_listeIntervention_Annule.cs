...
{
    public partial class Form_listeIntervention_Annule : Form
    {
        public Form_listeIntervention_Annule()
        {
            InitializeComponent();
        }
        Class1 c = new Class1();

        private void Form_listeIntervention_Annule_Load(object sender, EventArgs e)
        {
            c.da3 = new SqlDataAdapter("select * from intervention where etat='Annule'", c.cnx);
            c.da3.Fill(c.ds, "int_a");
            dataGridView1.DataSource = c.ds.Tables["int_a"];
